...
/** RTM call manager methods. */
    ...
    public:
    /** Allows a local user (the caller) to send a call invitation to a remote user (the callee).
    
     @param invitation An ILocalCallInvitation object.
     */
      virtual int sendLocalInvitation(ILocalCallInvitation *invitation)= 0;
    /** Allows a local user (the callee) to accept a call invitation from a remote user (the caller).
     
     @param remoteInvitation An IRemoteCallInvitation object.
     */
      virtual int acceptRemoteInvitation(IRemoteCallInvitation *invitation) = 0;
    /** Allows a local user (the callee) to decline a call invitation from a remote user (the caller).
    
     @param remoteInvitation An IRemoteCallInvitation object.
     */
      virtual int refuseRemoteInvitation(IRemoteCallInvitation *invitation) = 0;
    /** Allows a local user (the caller) to cancel a call invitation to a remote user (the callee).
