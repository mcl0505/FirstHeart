...
public class BeibaoPagerAdapter extends PagerAdapter {
    ...
    public BeibaoPagerAdapter(Context context, List<GiftListBeanNew.DataBean.MyWaresBean> giftList) {
        ...
        BeibaoAdapter.ActionListener actionListener = new BeibaoAdapter.ActionListener() {
            @Override
            public void onCancel() {
                if (mPage >= 0 && mPage < mViewList.size()) {
                    BeibaoAdapter adapter = (BeibaoAdapter) mViewList.get(mPage).getAdapter();
                    if (adapter != null) {
        ...
        for (int i = 0; i < pageCount; i++) {
            ...
            List<GiftListBeanNew.DataBean.MyWaresBean> list = new ArrayList<>();
            for (int j = fromIndex; j < endIndex; j++) {
                GiftListBeanNew.DataBean.MyWaresBean bean = giftList.get(j);
                bean.setPage(i);
                list.add(bean);
            }
            BeibaoAdapter adapter = new BeibaoAdapter(context, inflater, list);
    ...
    public void release() {
        if (mViewList != null) {
            for (RecyclerView recyclerView : mViewList) {
                BeibaoAdapter adapter = (BeibaoAdapter) recyclerView.getAdapter();
                if (adapter != null) {
                    adapter.release();
                }
            }
    ...
    public interface ActionListener {
        void onItemChecked(GiftListBeanNew.DataBean.MyWaresBean bean, int position);
    }
